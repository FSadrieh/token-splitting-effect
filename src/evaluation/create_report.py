...
def main():
    ...
    for soft_prompt_class, [num_soft_prompts, config_path] in EXAMINED_SOFT_PROMPTS[args.dataset].items():
        ...
        class_model_loss = []
        for i in range(num_soft_prompts[0], num_soft_prompts[1]):
            print(f"Running evaluation for {soft_prompt_class}_{i}")
            soft_prompt_name = f"{soft_prompt_class}_{i}"
            model_config_path = DEFAULT_CONFIG_PATHS[args.dataset] + config_path + f"{soft_prompt_name}.yml"
            if i == 0 and ("init" in soft_prompt_name or "train" in soft_prompt_name):
                soft_prompt_name = "5_model_0" if args.dataset == "imdb" else "emotion_5_model_0"
                model_config_path = DEFAULT_CONFIG_PATHS[args.dataset] + "5_models/models/" + f"{soft_prompt_name}.yml"
            ...
            class_normal_length_tokens_loss.append(float(evaluator.normal_prompt_length_loss[-1]))
            class_shortend_prompt_loss.append(float(evaluator.shortend_loss[-1]))
    ...
    with open(
        ...
        writer = csv.writer(f)
        writer.writerow(["metrics"] + list(EXAMINED_SOFT_PROMPTS[args.dataset].keys()))
        ...
        writer.writerow(["avg_model_losses_for_worst_tokens"] + avg_model_losses)
