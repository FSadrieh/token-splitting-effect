...
class Token_Relevance_Evaluation():
    def __init__(self, soft_prompt_name: str, model_numbers: List[int], config: str, accelerator: str, prompt_length: int, embedding_size: int, k: int, batch_size: int):
    ...
    def loss_evaluation(self):
        self.model_per_token, self.model_loss_per_token = prompt_token_drop_out(
            self.soft_prompt_name, self.model_numbers, self.config, self.accelerator, self.prompt_length, self.embedding_size, self.batch_size
        )
        self.normal_prompt_length_loss = []
        self.shortend_loss = []
        for i, model_number in enumerate(self.model_numbers):
            tokens_to_drop = [j for j, x in enumerate(self.model_per_token) if x == i]
            prompt_shorten_args = {
            ...
            self.shortend_loss(validate_prompt(**prompt_shorten_args))

    def nn_evaluation(self):
        model_names = get_model_names_from_numbers(self.model_numbers)
        ...
        self.euc_accuracy = sum(self.euc_nn_votes[i][0] == self.model_per_token[i] for i in range(len(self.euc_nn_votes))) / len(self.euc_nn_votes)
        self.cos_accuracy = sum(self.cos_nn_votes[i][0] == self.model_per_token[i] for i in range(len(self.cos_nn_votes))) / len(self.cos_nn_votes)
        self.euc_waccuracy = sum((self.euc_nn_votes[i][0] == self.model_per_token[i]) * self.euc_nn_votes[i][1] for i in range(len(self.euc_nn_votes))) / sum(self.euc_nn_votes[i][1] for i in range(len(self.euc_nn_votes)))
        self.cos_waccuracy = sum((self.cos_nn_votes[i][0] == self.model_per_token[i]) * self.cos_nn_votes[i][1] for i in range(len(self.cos_nn_votes))) / sum(self.cos_nn_votes[i][1] for i in range(len(self.cos_nn_votes)))
        self.euc_to_cos_accuracy = sum(self.euc_nn_votes[i][0] == self.cos_nn_votes[i][0] for i in range(len(self.euc_nn_votes))) / len(self.euc_nn_votes)
    ...
    def save_results(self):
        with open(
            ...
            writer = csv.writer(f)
            writer.writerow(["seed"] + [i for i in range(len(self.prompt_length))])
            ...
            writer.writerow([""])
            writer.writerow(["Prompt compression"] + [model_name for model_name in self.model_names] + ["average"])
            writer.writerow(["Normal prompt length"] + [self.normal_prompt_length_loss])
            writer.writerow(["Shortened prompt length"] + [self.shortend_loss])

        # We want to save the average normal length prompt loss and the average shortened prompt loss
        


    def nearest_neighbor_vote(self, distance_metric: str, soft_prompt_weight: torch.Tensor, model_embedding_spaces: list, labels: list) -> List[int]:
        nearest_neighbors = get_k_nearest_neighbors_for_all_tokens(distance_metric, soft_prompt_weight, model_embedding_spaces, self.k)
        nn_votes = []
    ...
    def read_from_csv(self):
        with open(
                ...
                self.euc_accuracy = float(rows[6][1])
                self.cos_accuracy = float(rows[7][1])
                self.euc_waccuracy = float(rows[8][1])
                self.cos_waccuracy = float(rows[9][1])
                self.euc_to_cos_accuracy = float(rows[10][1])
                self.normal_prompt_length_loss = rows[13][1:]
                self.shortend_loss = rows[14][1:]
